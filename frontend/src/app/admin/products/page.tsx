...
export default function AdminProducts() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const url = editingProduct 
      ? `${process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000"}/api/products/${editingProduct.id}`
  ...
  const handleDelete = async (id: string) => {
    ...
    const token = localStorage.getItem("zenvyra_token");
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000"}/api/products/${id}`, {
        method: "DELETE",
